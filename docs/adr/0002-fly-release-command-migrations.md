...
Schema Migrations run as Fly `release_command` (`npm run prisma:migrate` → `prisma migrate deploy`) before new machines receive traffic — not in the container `CMD`/entrypoint alongside the HTTP server.
